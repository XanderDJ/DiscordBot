...
removeDefCommand m = do
    ...
    ifElse allowed (removeRole m) (sendMessage $ R.CreateMessage (messageChannel m) (append (pingUserText m) ", you do not have the permission to run this command."))
...
removeRoles [] _ m = sendMessage $ R.CreateMessage (messageChannel m) (append (pingUserText m) ", No roles were present in the message! you need to @role!")
removeRoles rIds gId m = do
    con <- lift getDbConnEnv
    if isNothing con
        then sendMessage $ R.CreateMessage (messageChannel m) (append (pingUserText m) ", couldn't connect to the database!")
        else do

...
type Category = Text -- ^ Title case, "Status", "Special", "Physical" (Will always ensure first letter is capitalised when accepted)
type Coverage = Text -- ^ Title Case
...
joinPokemonMove pokemon move pmr = where_ $ pokemonId pokemon .== pmPokemonId pmr .&& moveId move .== pmMoveId pmr
...
hasMove con mon move = not . null <$> (runSelect con $ hasMoveSelect mon move :: IO [DBPokemon])
...
hasMoves con pId mIds = (\moves -> length moves == length mIds) <$> (runSelect con $ hasMovesSelect pId mIds :: IO [DBMove])
...
selectPokemonAbilities pokemon = do
    ...
    where_ $ pokemonId mon .== paPokemonId pma .&& paAbilityId pma .== abilityId ability
    pure ability
...
getCompletePokemon :: Connection -> PokemonId -> IO (Either Text DBCompletePokemon)
getCompletePokemon con mon = do
    ...
    let pl = length pokemon
        ...
        al = length abilities
        ml = length moves
    if pl /= 1 || tl == 0 || al == 0 then return (Left mon) else return $ Right (head pokemon, map typeName types, map abilityName abilities, moves)
...
joinPokemonAbility :: T.PokemonF -> T.AbilityF -> T.PokemonAbilityF -> Select ()
joinPokemonAbility mon ability par = where_ $ pokemonId mon .== paPokemonId par .&& abilityId ability .== paAbilityId par

getPokemonsWithAbility :: Connection -> AbilityId -> IO [DBPokemon]
getPokemonsWithAbility con ability  = runSelect con $ selectPokemonWithAbility ability

selectPokemonsWithStat :: (T.PokemonF -> Column SqlInt4) -> Int -> Select T.PokemonF
...
getPokemonsWithStat con "atk" stat = Just <$> runSelect con (selectPokemonsWithStat pokemonAtk stat)
getPokemonsWithStat con "def" stat = Just <$> runSelect con (selectPokemonsWithStat pokemonDef stat)
getPokemonsWithStat con "spa" stat = Just <$> runSelect con (selectPokemonsWithStat pokemonSpa stat)
getPokemonsWithStat con "spd" stat = Just <$> runSelect con (selectPokemonsWithStat pokemonSpd stat)
getPokemonsWithStat con "spe" stat = Just <$> runSelect con (selectPokemonsWithStat pokemonSpe stat)
getPokemonsWithStat con "hp"  stat = Just <$> runSelect con (selectPokemonsWithStat pokemonHp stat)
...
selectCoverageMoves :: PokemonId -> Coverage -> Select T.MoveF
selectCoverageMoves pId tipe = do
    move <- selectPokemonMoves pId
    isMoveType move tipe
    return move

getCoverageMoves :: Connection -> PokemonId -> Coverage -> IO [DBMove]
getCoverageMoves con mon tipe = runSelect con $ selectCoverageMoves mon tipe

isMoveType :: T.MoveF -> Coverage -> Select ()
isMoveType move tipe = where_ $ moveType move .== toFields (toTitle tipe)


selectCategoryMoves :: PokemonId -> Category -> Select T.MoveF
selectCategoryMoves pId category = do
    move <- selectPokemonMoves pId
    isMoveCategory move category
    return move

isMoveCategory :: T.MoveF -> Category -> Select ()
isMoveCategory move category = where_ $ moveCategory move .== toFields (toTitle category)

getCategoryMoves :: Connection -> PokemonId -> Category -> IO [DBMove]
getCategoryMoves con pId category = runSelect con $ selectCategoryMoves pId category

selectCategoryCoverageMoves :: PokemonId -> Category -> Coverage -> Select T.MoveF
...
getCategoryCoverageMoves :: Connection -> PokemonId -> Category -> Coverage -> IO [DBMove]
getCategoryCoverageMoves con pId category coverage = runSelect con $ selectCategoryCoverageMoves pId category coverage
...
getObjectClass con oId = (\l -> if length l == 1 then Just (head l) else Nothing) <$> runSelect con (selectObjectClass oId)


getData :: Connection -> ObjectId -> IO (Maybe DBData)
getData con oId = do
    oc <- getObjectClass con oId
    if isNothing oc then return Nothing else getDt con oId ((ocClass . fromJust) oc)
...
getDt con oId "pokemon" = do
    ...
    return $ if isLeft mon then Nothing else (Just . DTP . extractRight) mon 

...
calculateTries p desiredSuccessRate = ceiling $ logBase (1-p) (1- desiredSuccessRate)
...
chanceForCl cl tries  = go cl tries 1
  where
    ...
    go cl try pf | try == 0  = 1 - pf
                 | otherwise =  let pf' = pf * (1 - getChance cl try 0) in go cl (try - 1) pf'
